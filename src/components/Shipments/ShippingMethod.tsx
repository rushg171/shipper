        ...
        <div className="flex-col p-6">
            <div className="flex w-full justify-between">
                ...
                <button className="pr-6 " onClick={()=>setIsCollapsed(!isCollapsed)}>
                    {!isCollapsed?<span className="text-blue-400">Collapse</span>:<span className="text-blue-800">Expand</span>}
                </button>
            </div>
                    ...
                    {shippingDetails.map(item=>{
                        return (
                            <div key={item.title} className="flex-col px-6 border-b-1 border-neutral-200 py-4">
                                ...
                                {item.icon && <Image src={`icons/${item.icon}`} alt={item.title} width={172} height={27}/>} 
                            </div>
                        )
                    })}
                </div>
